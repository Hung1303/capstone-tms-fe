...
const StaffCourseConfirmation = () => {
  ...
  const fetchAssignedCourses = async () => {
    setLoading(true)
    try {
      const response = await api.get('/Course?status=ASSIGNED&pageSize=1000')
  ...
  const fetchTeachers = async () => {
    try {
      const response = await api.get('/User?role=Teacher&pageSize=1000')
      const teacherList = response?.data?.data || response?.data || []
  ...
  const getTeacherName = (teacherId) => {
    ...
    return teacher?.fullName || teacher?.userName || 'N/A'
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Xác nhận khóa học</h2>
          <p className="text-gray-600">Xem và xác nhận các khóa học đã được giáo viên ký</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
          ...
          <div className="text-sm text-gray-600">Khóa học chờ xác nhận</div>
        </div>
      </div>
              ...
              {filteredCourses.length > 0 ? (
                filteredCourses.map((course) => (
                  <tr key={course.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm text-gray-900">{getTeacherName(course.teacherProfileId)}</div>
                    </td>
                    <td className="px-6 py-4">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-900">{getTeacherName(selectedCourse.teacherProfileId)}</p>
                </div>
              </div>
            ...
            <div className="p-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-lg font-semibold text-gray-900">
                  {confirmAction === 'approve' ? 'Xác nhận khóa học' : 'Từ chối khóa học'}
                </h3>
              </div>
              <p className="text-gray-600 mb-6">
                {confirmAction === 'approve' 
                  ? `Bạn có chắc chắn muốn xác nhận khóa học "${selectedCourse.title}"?`
              ...
              <div className="flex gap-3 justify-end">
                ...
                >
                  {loading ? 'Đang xử lý...' : confirmAction === 'approve' ? 'Xác nhận' : 'Từ chối'}
                </button>
              </div>
            </div>
          </div>
...
export default StaffCourseConfirmation
